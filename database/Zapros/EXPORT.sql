...
COPY (
SELECT *  FROM "USER_NAME" WHERE USER_N_ID = '996MdkMfxhWRZTD8G0F3'
) 
...
COPY (
SELECT * FROM "USER_PRIVATE" WHERE USER_P_ID = 'udP1PlxlVqsWO0N99aVA'
...
COPY (
SELECT "USER".USER_ID , CAT_NAME  FROM "USER" LEFT JOIN  "CATEGORY" ON "USER".USER_ID="CATEGORY".USER_ID WHERE "USER".USER_ID = 1
) 
...
COPY (
SELECT "USER".USER_ID, ACTL_NAME,  CAT_NAME   
FROM "USER"  LEFT JOIN "ACTIVITY_LIST" ON "USER".USER_ID="ACTIVITY_LIST".USER_ID  WHERE "USER".USER_ID = 1
) 
